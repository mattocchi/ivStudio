...
@ManagedBean
@ViewScoped
public class BeanLinee extends PageBaseListS74<Linea, LineaFiltro> {
	...
	protected void onPostLoadData() throws Exception {
		...
		cacheModuli = new ModuloAPI().lista(getEmfShared(), getCurrentUser());
	}
	...
	public List<Modulo> elencoModuli(String ids) {
		List<Modulo> res = new ArrayList<Modulo>();
		try {

			// res = new ModuloAPI().listaDaIds(getEmfShared(), ids);
			res = ListUtils.findByIdsLong(cacheModuli, ids);
